...
y_train = np.random.randint(0, 5, size=num_samples)
X_test = np.random.rand(100, 224, 224, 3)

# Existing CNN model
cnn = keras.applications.ResNet50(weights='imagenet')  

# Get train and test CNN embeddings
cnn_train = cnn(X_train)
cnn_test = cnn(X_test)
...
# SVM model
svm_model = svm.SVC()
svm_model.fit(cnn_train, y_train)  

# Universal gradient wrapper
@tf.custom_gradient
def svm_grad(x):
    def grad(dy):
        return tf.gradients(svm_model(x), x) * dy  
    return svm_model(x), grad

# Calculate gradients
grad = svm_grad(cnn_test) 
